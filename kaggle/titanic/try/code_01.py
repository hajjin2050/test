...
#데이터 불러오기
train_df = pd.read_csv('C:\workspace/titanic\data/train.csv')
test_df = pd.read_csv('C:\workspace/titanic\data/test.csv')
submission = pd.read_csv('C:\workspace/titanic\data/sample_submission.csv')
test_df[TARGET] = pd.read_csv("C:\workspace/titanic\data/pseudo_label.csv")[TARGET]

#트레인 데이터와 테스트 데이터 합치기  
...
submission['submit_2'] = pd.read_csv("C:\workspace/titanic\data/dae.csv")[TARGET]
submission['submit_3'] = pd.read_csv("C:\workspace/titanic\data/pseudo_label.csv")[TARGET]

submission[[col for col in submission.columns if col.startswith('submit_')]].sum(axis = 1).value_counts()

submission[TARGET] = (submission[[col for col in submission.columns if col.startswith('submit_')]].sum(axis=1) >= 2).astype(int)

submission[['PassengerId', TARGET]].to_csv("C:\workspace/titanic\data/voting_submission4.csv", index = False)