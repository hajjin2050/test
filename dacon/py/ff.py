...
for q in qlist:
    ...
    filepath = f'C:/data/dacon/modelcheckpoint/dacon_train_0126_0_y1_{q:.1f}.hdf5'  #앞에 f를 붙여준 이유: {}안에 변수를 넣어주겠다는 의미
...
for q in qlist:
    ...
    stop = EarlyStopping(monitor ='val_loss', patience=patience, mode='min')
    reduce_lr = ReduceLROnPlateau(monitor='val_loss', patience=patience/2, factor=0.5, verbose=1)
    filepath = f'C:/data/dacon/modelcheckpoint/dacon_train_0126_1_y2_{q:.1f}.hdf5'
...
subfile.to_csv('C:/data/dacon/final_submit/sub_0126_1.csv', index=False)