...
train_x = train_x.reshape(train_x[0], 18,1)
model = Sequential()
model.add(LSTM(1280,input_shape=(9,), activation='relu'))``
...
dangjin_floating_model = model.fit(train_x, train_y, validation_split = 0.2, epochs = 80, batch_size = 32)
dangjin_floating_model = model.predict(val_x)
'''
...
capacity = 700
dangjin_warehouse_model = model.predict(train_x, train_dataset, 10000, val_dataset, feval=nmae_10, verbose_eval=500, early_stopping_rounds=100)
