...
for i in range(48):
    ...
    for j in quantiles:
        model = mymodel()
        filepath_cp = f'C:/data/dacon/modelcheckpoint/dacon_09/dacon_09_{i:2d}_y1seq_{j:.1f}.hdf5'
    ...
    for j in quantiles:
        model = mymodel()
        filepath_cp = f'C:/data/dacon/modelcheckpoint/dacon_09/dacon_09_{i:2d}_y2seq_{j:.1f}.hdf5'
        cp = ModelCheckpoint(filepath_cp,save_best_only=True,monitor = 'val_loss')
        model.compile(loss = lambda y_true,y_pred: quantile_loss(j,y_true,y_pred), optimizer = 'adam', metrics = [lambda y,y_pred: quantile_loss(j,y,y_pred)])
