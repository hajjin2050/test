...
def split_to_seq(data): 
    tmp = []
    for i in range(48): #30분단위로 하루를 쪼개면 48개
        tmp1 = pd.DataFrame()
        for j in range(int(len(data)/48)):
            tmp2 = data.iloc[j*48+i,:]
            tmp2 = tmp2.to_numpy()
            tmp2 = tmp2.reshape(1,tmp2.shape[0]) #나열되어있는 시계열 데이터로 만드려고
...
def preprocess_data(data, is_train = True):
    ...
    for i in range(int(len(data)/48)):
        tmp = pd.DataFrame()
        tmp['cos'] = data.loc[i*48:(i+1)*48-1,'TARGET'] #코스로 타겟값 구하고
    ...
    dp = ( c * gamma) / (b - gamma)
    data.insert(1,'Td',dp) #어떤 방식으로 인서트함수가 컬럼을 추가할까? 티디각 뭘까
...
x,y1,y2 = [],[],[]
for i in range(48):
    tmp1,tmp2,tmp3 = split_xy(train[i],day)#day = 7
    x.append(tmp1)
    y1.append(tmp2)
    y2.append(tmp3)
...
submission.to_csv('c:/data/dacon/final_submit/0122_1.csv', index = False)