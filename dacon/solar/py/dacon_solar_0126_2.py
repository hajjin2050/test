...
for i in range(48):
    ...
    # 내일!
    for j in quantiles:
        filepath_cp = f'C:/data/dacon/modelcheckpoint/dacon_0126_2_{i:2d}_y1seq_{j:.1f}.hdf5'
    ...
    # 모레!
    for j in quantiles:
        filepath_cp = f'C:/data/dacon/modelcheckpoint/dacon_0126_2_{i:2d}_y2seq_{j:.1f}.hdf5'
        model = load_model(filepath_cp, compile = False)
        model.compile(loss = lambda y_true,y_pred: quantile_loss(j,y_true,y_pred), optimizer = 'adam', metrics = [lambda y,y_pred: quantile_loss(j,y,y_pred)])
        x = []
        for k in range(81):
...
submission.to_csv('C:/data/dacon/final_submit/dacon_0126_2.csv', index = False)