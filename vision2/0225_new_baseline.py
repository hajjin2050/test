...
labels_df = pd.read_csv('C:\data\dacon\mnist2\dirty_mnist_2nd_answer.csv')[:]
imgs_dir = np.array(sorted(glob.glob('C:\data\dacon\mnist2\dirty_mnist_2nd/*')))[:]
labels = np.array(labels_df.values[:,1:])

test_imgs_dir = np.array(sorted(glob.glob('C:\data\dacon\mnist2\dirty_mnist_2nd/*')))
...
#     model = nn.DataParallel(model)
    ...
    epochs=25
    batch_size = 16      # 자신의 VRAM에 맞게 조절해야 OOM을 피할 수 있습니다.
    ...
    for epoch in range(epochs):
        ...
        if np.mean(valid_batch_accuracy)>valid_best_accuracy:
            torch.save(model.state_dict(), 'C:/data/dacon/mnist2/mc/EfficientNetB0-fold{}.pt'.format(fold))
            valid_best_accuracy = np.mean(valid_batch_accuracy)
        print('fold : {}/tepoch : {:02d}/ttrain_accuracy / loss : {:.5f} / {:.5f}/tvalid_accuracy / loss : {:.5f} / {:.5f}/ttime : {:.0f}'.format(fold+1, epoch+1,
...
#테스트 추론
submission = pd.read_csv('C:/data/dacon/mnist2/dataset/sample_submission.csv')

with torch.no_grad():
    for fold in range(1):
        model = EfficientNet_MultiLabel(in_channels=3).to(device)
        model.load_state_dict(torch.load('C:/data/dacon/mnist2/mc/EfficientNetB0-fold{}.pt'.format(fold)))
        model.eval()
...
submission.to_csv('C:/data/dacon/mnist2/EfficientNetB0-fold0.csv', index=False)