...
with torch.no_grad():
    for fold in range(1):
        model = EfficientNet_MultiLabel(in_channels=3).to(device)
        model.load_state_dict(torch.load('C:/data/dacon/mnist2/02287EfficientNetB5-fold1.pt'. format(fold)))
        model.eval()
...
submission.to_csv('C:/data/dacon/mnist2/0228_0_base.csv', index=False)