...
x = []
for i in range(2,5):
    df = pd.read_csv(f'C:/data/dacon/mnist2/0301_6_base.csv', index_col=0, header=0)
    data = df.to_numpy()
    x.append(data)

x = np.array(x)

df = pd.read_csv(f'C:/data/dacon/mnist2/0301_b4_base.csv', index_col=0, header=0)
for i in range(5000):
    for j in range(26):
        a = []
        for k in range(3):
...
y.to_csv('C:/data/dacon/mnist2/0301_final_base.csv')  