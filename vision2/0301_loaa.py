...
test_imgs_dir = np.array(sorted(glob.glob('C:/data/dacon/mnist2/test_dirty_mnist_2nd/*')))
...
#     epochs=8
#     batch_size=6        # 자신의 VRAM에 맞게 조절해야 OOM을 피할 수 있습니다.
    ...
    #         torch.save(model.state_dict(), 'C:/data/dacon/mnist2/0301_EfficientNetB4-fold{}.pt'. format(fold))
...
with torch.no_grad():
    for fold in range(1):
        model = EfficientNet_MultiLabel(in_channels=3).to(device)
        model.load_state_dict(torch.load('C:/data/dacon/mnist2/0301_EfficientNetB4-fold0.pt'. format(fold)))
        model.eval()
...
submission.to_csv('C:/data/dacon/mnist2/0301_b4_base.csv', index=False)