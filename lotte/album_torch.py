...
height = 128
color = 3 #색수
x_data = np.load('C:/workspace/lotte/x_0318', allow_pickle=True)
y_data = np.load('C:/workspace/lotte/y_0318', allow_pickle=True)
...
x_train, x_val, y_train, y_val = train_test_split(x_data, y_data, train_size=0.3, random_state=777, stratify=y_data)
...
model.fit(x_train, y_train, epochs=3, batch_size=32, shuffle=True, validation_data=(x_test, y_test), callbacks=[ModelCheckpoint(filepath='model/figure_shape_classification_model.h5', save_best_only=True, verbose=1)]) 
...
model = load_model('model/figure_shape_classification_model.h5')
