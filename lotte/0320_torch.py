...
BATCH_SIZE = 16
EPOCHS = 200
TRAIN_PATH = 'C:/workspace/lotte/train_new/'
PRED_PATH = 'C:/workspace/lotte/test_new/'
transformer = transforms.Compose([ # 파일가져오기 형식들
...
for i in range(72000):
    image = Image.open(f'C:/workspace/lotte/test/{i}.jpg')
    image.save('C:/workspace/lotte/test_new/{0:05}.jpg'.format(i))
...
for epoch in tqdm(range(num_epochs)):
    ...
    if test_accuracy>best_accuracy:
        torch.save(model.state_dict(),'C:/workspace/lotte/h5/best_checkpoint2.pth')
        best_accuracy=test_accuracy

PATH = 'C:/workspace/lotte/h5/best_checkpoint2.pth'
...
def predict(model, path, sample_size=72000):
    for file in tqdm(glob.glob(os.path.join('C:/data/LPD_competition/test_new/*.jpg'))[:sample_size]):
...
submission.to_csv('C:/workspace/lotte/submit/pytorch_result2.csv', index=False)
